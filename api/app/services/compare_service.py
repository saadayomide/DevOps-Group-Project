...
class CompareService:
    ...
    def find_best_matches(
        self, query: str, products: List[dict], threshold: Optional[float] = None
    ) -> List[dict]:
        """
        ...
        for product in products:
            ...
            if similarity >= threshold:
                matches.append({**product, "similarity": similarity})
    ...
    def calculate_price_difference(
        self, comparison1: ProductComparison, comparison2: ProductComparison
    ) -> float:
    ...
    def get_price_statistics(self, comparisons: List[ProductComparison]) -> dict:
        ...
        if not comparisons:
            return {"count": 0, "min": 0.0, "max": 0.0, "average": 0.0, "median": 0.0}
        ...
        return {
            ...
            "median": (
                prices[count // 2]
                if count % 2 == 1
                else (prices[count // 2 - 1] + prices[count // 2]) / 2
            ),
        }

    def compare_basket(self, request: CompareRequest, db: Session) -> CompareResponse:
        """
        ...
        for product in all_products:
            ...
            if (
                ...
                and normalized_product_name not in normalized_product_name_to_product
            ):
        ...
        if not store_objs:
            ...
            return CompareResponse(
                items=[], storeTotals=[], overallTotal=0.0, unmatched=request.items
            )
        ...
        if product_ids and store_ids:
            prices = (
                db.query(Price)
                .filter(Price.product_id.in_(product_ids), Price.store_id.in_(store_ids))
                .all()
            )
        ...
        for item in request.items:
            ...
            min_price_stores = [
                (store_id, price) for store_id, price in candidates if price == min_price
            ]
            ...
            compare_items.append(
                CompareItem(name=item, store=selected_store_name, price=float(selected_price))
            )
    ...
    def compare_items_across_stores(
        self, items: List[str], stores: List[str], item_prices: Dict[Tuple[str, str], float]
    ) -> CompareResponse:
        """
        ...
        for item in items:
            ...
            for store in stores:
                ...
                if key in item_prices:
                    ...
                    compare_items.append(CompareItem(name=item, store=store, price=price))
        ...
        store_totals_list = [
            StoreTotal(store=store, total=total) for store, total in store_totals.items()
        ]
